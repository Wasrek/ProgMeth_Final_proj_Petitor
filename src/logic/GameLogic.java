...
public class GameLogic {
	...
	public void useEffClick() {
		...
		for(BaseCard cd : this.getnowClick()) {
			if(cd.isEffectable()) cd.useEffect();
		}
		this.getnowClick().clear();
		((GameFieldPane) main.getGameRoot().getChildren().get(1)).updateField();
		((LeftGamePane) main.getGameRoot().getChildren().get(0)).updateStatus();
	...
	public void useEffClick(int choosen) {
		if(this.getnowClick().size()!=choosen) return ;
		if(this.getnowClick().get(0).getOwner()==this.getCurrentPlayer()) {
			if(this.getnowClick().get(0).isEffectable()){
				((MonsterCard) this.getnowClick().get(0)).attack(this.getnowClick().get(1));
			}
		}else {
			if(((MonsterCard) this.getnowClick().get(1)).isAttackable(this.getnowClick().get(0))){
				((MonsterCard) this.getnowClick().get(1)).attack(this.getnowClick().get(0));
			}
		}
		this.getnowClick().clear();
		((GameFieldPane) main.getGameRoot().getChildren().get(1)).updateField();
