...
//    	int ty = 1;
    	...
    	if(ty == 2) {
    		if(!m) return false;
    		if(GameLogic.getInstance().getCurrentPlayer()==GameLogic.getInstance().getnowClick().get(0).getOwner()) return false;
    	}
    	//one spell
    	if(ty == 3) {
    		return false;
    ...
    private void EffClicked(MouseEvent e, int i,int p,boolean m) {
    	...
    	if(GameLogic.getInstance().getnowClick().size()==0 && GameLogic.getInstance().getCurrentPlayer()!=p) return ;
    	BaseCard nowCard;
    	if(m) {
